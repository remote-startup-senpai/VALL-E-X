...
checkpoint = torch.load(
    "./checkpoints/vallex-checkpoint.pt", map_location="cpu"
)
...
try:
    ...
    raise Exception(
        ...
        "\n manually download model and put it to {} .".format(
            os.getcwd() + "\whisper"
        )
    )
...
preset_list = [
    preset[:-4] for preset in preset_list if preset.endswith(".npz")
]
...
def make_npz_prompt(name, uploaded_audio, recorded_audio, transcript_content):
    ...
    clear_prompts()
    audio_prompt = (
        uploaded_audio if uploaded_audio is not None else recorded_audio
    )
...
def infer_from_audio(
    ...
    global model, text_collater, text_tokenizer, audio_tokenizer
    audio_prompt = (
        audio_prompt if audio_prompt is not None else record_audio_prompt
    )
    sr, wav_pr = audio_prompt
    if not isinstance(wav_pr, torch.FloatTensor):
        wav_pr = torch.FloatTensor(wav_pr)
...
def infer_from_prompt(text, language, accent, preset_prompt, prompt_file):
    ...
    if prompt_file is not None:
        ...
        prompt_data = np.load(
            os.path.join("./presets/", f"{preset_prompt}.npz")
        )
...
def infer_long_text(
    ...
    if (prompt is None or prompt == "") and preset_prompt == "":
        mode = (
            "sliding-window"  # If no prompt is given, use sliding-window mode
        )
    ...
    if prompt is not None and prompt != "":
        ...
        # numpy to tensor
        audio_prompts = (
            torch.tensor(audio_prompts).type(torch.int32).to(device)
        )
        text_prompts = torch.tensor(text_prompts).type(torch.int32)
        ...
        audio_prompts = (
            torch.zeros([1, 0, NUM_QUANTIZERS]).type(torch.int32).to(device)
        )
        text_prompts = torch.zeros([1, 0]).type(torch.int32)
        lang_pr = language if language != "mix" else "en"
    if mode == "fixed-prompt":
        complete_tokens = (
            torch.zeros([1, NUM_QUANTIZERS, 0])
        ...
        for text in sentences:
            ...
            enroll_x_lens = text_prompts.shape[-1]
            logging.info(f"synthesize text: {text}")
            phone_tokens, langs = text_tokenizer.tokenize(
                text=f"_{text}".strip()
            )
            text_tokens, text_tokens_lens = text_collater([phone_tokens])
            text_tokens = torch.cat([text_prompts, text_tokens], dim=-1)
